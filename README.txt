...
5.5 Create a list of bam files. ls GATK_STEPS/*realigned.bam >bam.list

6 Program "mgSNP_gatk-SNP.sh" - Calls SNP (VCF files) using GATK for each samples and for each genome seperately. The program create a script file and SLURM job file for each genome (each genome file has commands to process all samples). The Jobs files created are not submitted on the fly (default).

  Run as run_mgSNP_gatk-SNP.sh to run all the commands. Make changes if required.
...
2. The genome and sample for which pairwise calculation needs to be done is put on a file name filtered.txt (the firt two columns of allcov.stats).
  The format is like genome name and sample name seperated by tab.
  Acidaminococcus_sp_D21  JG5012A
  Acidaminococcus_sp_D21  JG5028A
  Acidaminococcus_sp_D21  JG5028C
  Akkermansia_muciniphila E02-PRE
  Akkermansia_muciniphila E06-1Y
  Akkermansia_muciniphila E06-3MDR
  Akkermansia_muciniphila F05-3MDR
  Akkermansia_muciniphila F11-1Y
  Akkermansia_muciniphila F11-3MDR
  Akkermansia_muciniphila JG5028B

  To make all pairwise comparision of all samples for all genomes, you can generate filtered.txt using this command
  ...
  Now the format (each species is in one line) looks like
  Acidaminococcus_sp_D21 JG5012A JG5028A JG5028C
  Akkermansia_muciniphila E02-PRE E06-1Y E06-3MDR F05-3MDR F11-1Y F11-3MDR JG5028B
